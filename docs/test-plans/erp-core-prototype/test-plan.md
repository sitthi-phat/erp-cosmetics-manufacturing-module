...
# 10. Gate 2 Round 2 — Test Plan (Q8–Q11) — 2026-07-07

> **บริบท**: ปอนด์ทดสอบ localhost แล้วไม่ approve Gate 2 (feedback 10 ข้อ, ดู
> `docs/requirements/erp-core-prototype/pond-gate2-feedback.md`) → กลายเป็น stories ใหม่/แก้ 44
> stories/14 epics + tasks ใหม่ Q8–Q11 (ดู `docs/design/erp-core-prototype/tasks.md` rev.3 §C).
> เขียนคู่ขนานกับ Engineer (E22–E33 กำลังทำอยู่ ณ ตอนเขียนเอกสารนี้) — **ตาม policy เดิม (§0/§1): ไม่รอโค้ด,
> เขียนจาก AC + contract (architecture.md rev.3 §13, ADR-009) ก่อน แล้ว reconcile ตอน verify**.
> Section นี้เป็น delta เพิ่มเติมจาก §1–§9 ด้านบน (ของเดิมยังมีผลทั้งหมด ไม่มีอะไรถูกลบ/ทับ)

## 10.1 Traceability Matrix — AC ใหม่/แก้ (Gate 2 Round 2) → TC ใหม่

Legend เหมือน §2: Level **U**=Unit, **I**=Integration, **E**=E2E, **M**=Manual/UAT.
Automatable: **Y**=ใช่, **P**=Partial, **N**=ไม่ได้ (เหตุผลในช่องหมายเหตุ).

### Epic 1: Customer (แก้ไข — tax fields)

| AC | TC ID | Level | Auto | ไฟล์ |
|---|---|---|---|---|
| ECP-001 AC5 (tax_id+registered_address บันทึกสำเร็จ) | TC-Q9-TAX-01 | I | Y | `tests/integration/customerTaxFields.spec.ts` |
| ECP-001 AC6 (tax_id ไม่ครบ 13 หลัก/มีตัวอักษร→error) | TC-Q8-TAX-02..04, TC-Q9-TAX-02/02b | U/I | Y | `tests/unit/taxIdValidation.spec.ts` + integration เดียวกัน |
| ECP-001 AC7 (tax_id ไม่บังคับตอนสร้าง) | TC-Q9-TAX-03 | I | Y | เดียวกัน |
| ECP-002 AC4 (tax_id snapshot ไม่แก้ย้อนหลัง) | TC-Q9-TAX-04 | I | P | เดียวกัน — โครงพร้อมแล้ว แต่ full assertion รอ E30/E32 (fixture helper เต็มรูปอยู่ที่ `invoiceDocument.spec.ts` TC-Q9-DOC-07 แทน) |

### Epic 2: PO (แก้ไข — line display + delete, regression guard หลัก)

| AC | TC ID | Level | Auto | ไฟล์ |
|---|---|---|---|---|
| ECP-004 AC1 (ชื่อสินค้า/จำนวน/ราคา/ยอดรวม, ห้าม `Product #<id>...`) | TC-Q10-A | E | Y | `tests/e2e/gate2RegressionGuard.spec.ts` (regression guard A) |
| ECP-004 AC2 (ลบบรรทัดก่อนยืนยัน) | TC-Q10-A | E | Y | เดียวกัน |
| ECP-004 AC5 (ลบบรรทัดหลัง Confirmed→block) | — | I | N (ยังไม่เขียน) | **GAP**: ต้องเพิ่ม integration test แยกสำหรับ AC5 โดยเฉพาะ (delete-line guard ที่ BE) — ระบุไว้ที่ §10.4 ว่ายังไม่ได้เขียนรอบนี้ เนื่องจากเวลาจำกัด ให้ priority สูงสุดตอน verify |

### Epic 3: Stock (แก้ไข — search)

| AC | TC ID | Level | Auto | ไฟล์ |
|---|---|---|---|---|
| ECP-007 AC4 (ค้นหา case-insensitive ≤2 วิ) | TC-Q10-B | E | Y | `tests/e2e/gate2RegressionGuard.spec.ts` (regression guard B) |
| ECP-007 AC5 (ไม่พบ→empty-state+ล้างการค้นหา) | TC-Q10-B | E | Y | เดียวกัน |
| ECP-008 AC4 (supplier_name ผูก Lot) | TC-Q9-SUPP-01 | I | Y | `tests/integration/goodsReceiptSupplier.spec.ts` |

### Epic 4: Production & Traceability (แก้ไข — auto-calc/FIFO, auto-detect)

| AC | TC ID | Level | Auto | ไฟล์ |
|---|---|---|---|---|
| ECP-013 AC1 (auto-calc required qty) | TC-Q8-FIFO-*, TC-Q9-PLAN-01 | U/I | Y | `tests/unit/fifoAllocation.spec.ts`, `tests/integration/productionMaterialPlan.spec.ts` |
| ECP-013 AC1 (FIFO เรียง received_date) | TC-Q8-FIFO-01/02, TC-Q9-PLAN-02 | U/I | Y | เดียวกัน |
| ECP-013 AC2 (L-SEED-1 ใช้งานได้จริง, regression guard D) | TC-Q9-PLAN-03, TC-Q10-D | I/E | Y | เดียวกัน + `gate2RegressionGuard.spec.ts` |
| ECP-013 AC3 (multi-lot split) | TC-Q8-FIFO-03, TC-Q9-PLAN-08 | U/I | Y | เดียวกัน |
| ECP-013 AC4 (ไม่มี BOM→409) | TC-Q9-PLAN-04 | I | Y | เดียวกัน |
| ECP-013 AC5 (server re-validate Σ=required พอดี) | TC-Q9-PLAN-05/06 | I | Y | เดียวกัน |
| ECP-017 AC1 (ฟอร์มขาเข้าดึง qty/lot/supplier อัตโนมัติ) | TC-Q9-SUPP-02 | I | Y | `tests/integration/goodsReceiptSupplier.spec.ts` |
| ECP-017 AC2/AC3 (Lot ไม่ Passed ถูกบล็อกแม้ผ่าน API ตรง) | TC-Q9-PLAN-07 | I | Y | `tests/integration/productionMaterialPlan.spec.ts` |
| ECP-017 AC4 (Lot เก่าไม่มี supplier→"ไม่ระบุ") | TC-Q9-SUPP-03 | I | Y | `tests/integration/goodsReceiptSupplier.spec.ts` |
| ECP-014 AC1 (auto-detect + L-SEED-1 หาเจอ, regression guard C) | TC-Q8-TRACE-*, TC-Q9-TRACEQ-01, TC-Q10-C | U/I/E | Y | `tests/unit/traceAutoDetect.spec.ts`, `tests/integration/traceAutoDetect.spec.ts`, `gate2RegressionGuard.spec.ts` |
| ECP-014 AC2 (auto-detect ทุกประเภท→สายเดียวกัน) | TC-Q8-TRACE-01..04, TC-Q9-TRACEQ-02..04 | U/I | Y | เดียวกัน |
| ECP-014 AC3 (legend Lot vs Batch) | TC-Q10-C | E | Y | `gate2RegressionGuard.spec.ts` |
| ECP-014 AC4 (Lot ใช้หลาย Batch→ครบ) | (ใช้ TC-014-AC2 เดิมที่มีอยู่แล้วใน §2) | I | Y | `tests/integration/traceability.spec.ts` (ไม่ต้องเขียนซ้ำ — ครอบคลุมแล้วตั้งแต่ Gate 1) |
| ECP-014 AC5 (ไม่พบ→ข้อความเป็นมิตร) | TC-Q8-TRACE-07, TC-Q9-TRACEQ-05 | U/I | Y | เดียวกัน |

### Epic 12: BOM Management (ใหม่ทั้งหมด)

| AC | TC ID | Level | Auto | ไฟล์ |
|---|---|---|---|---|
| ECP-039 AC1 (สร้าง BOM ใหม่) | TC-Q9-BOM-01, TC-Q10 (BOM smoke) | I/E | Y | `tests/integration/bom.spec.ts`, `gate2RegressionGuard.spec.ts` |
| ECP-039 AC2 (แก้ไข in-place ไม่มี version) | TC-Q9-BOM-03 | I | Y | `tests/integration/bom.spec.ts` |
| ECP-039 AC3 (ลบบรรทัด, ยืนยันก่อนลบ) | TC-Q9-BOM-05 | I | P | integration ครอบ business logic เต็ม; "ยืนยันก่อนลบ" (confirm dialog) เป็น FE UX ที่ยังไม่มี e2e เจาะจง — ดู §10.4 gap |
| ECP-039 AC4 (≥1 บรรทัดเสมอ) | TC-Q8-BOM-02/04, TC-Q9-BOM-04 | U/I | Y | `tests/unit/bomValidation.spec.ts`, `tests/integration/bom.spec.ts` |
| ECP-039 AC5 (ห้ามวัตถุดิบซ้ำ) | TC-Q8-BOM-03, TC-Q9-BOM-06 | U/I | Y | เดียวกัน |

### Epic 13: Thai Tax Invoice Document (ใหม่ทั้งหมด)

| AC | TC ID | Level | Auto | ไฟล์ |
|---|---|---|---|---|
| ECP-040 AC1 (invoice detail ครบ) | TC-Q9-DETAIL-01, TC-Q10-E | I/E | Y | `tests/integration/invoiceDiscountDetail.spec.ts`, `gate2RegressionGuard.spec.ts` |
| ECP-040 AC2 (version-aware, ไม่ปะปน) | TC-Q9-DETAIL-02 | I | Y | เดียวกัน |
| ECP-040 AC3 (id ไม่มีจริง→404) | TC-Q9-DETAIL-03 | I | Y | เดียวกัน |
| ECP-040 AC4 (RBAC 403) | TC-Q9-DETAIL-04 | I | Y | เดียวกัน |
| ECP-020 AC4 (discount math) | TC-Q8-DISC-01..04, TC-Q9-INVDISC-01 | U/I | Y | `tests/unit/invoiceDiscount.spec.ts`, `tests/integration/invoiceDiscountDetail.spec.ts` |
| ECP-020 AC5 (discount>subtotal→reject) | TC-Q8-DISC-05/05b, TC-Q9-INVDISC-02 | U/I | Y | เดียวกัน |
| ECP-041 AC1 (ตั้งค่า CompanyProfile ครั้งแรก) | TC-Q9-CP-01 | I | Y | `tests/integration/companyProfile.spec.ts` |
| ECP-041 AC2 (แก้ไข→snapshot เก่าไม่เปลี่ยน) | TC-Q9-CP-03, TC-Q9-DOC-07 | I | Y | `companyProfile.spec.ts` + `invoiceDocument.spec.ts` (proof เต็มรูปอยู่ที่ DOC-07) |
| ECP-041 AC3 (tax_id ไม่ครบ 13 หลัก→reject) | TC-Q8-TAX-*, TC-Q9-CP-04 | U/I | Y | `taxIdValidation.spec.ts`, `companyProfile.spec.ts` |
| ECP-041 AC4 (ไม่มี CompanyProfile→block พิมพ์) | TC-Q9-DOC-05 | I | Y | `tests/integration/invoiceDocument.spec.ts` |
| ECP-042 AC1 (เอกสารครบทุกส่วน ตรงตัวอย่าง) | TC-Q8-BAHT-02, TC-Q9-DOC-01, TC-Q10-E | U/I/E | P | automated ยืนยัน "ข้อมูล/ข้อความครบ" ได้เต็มที่ — **"ตรงตัวอย่าง 100% เชิงสายตา" เป็น manual เท่านั้น** ดู `uat-print-responsive-script.md` ส่วนที่ 1 |
| ECP-042 AC2 (discount=0 ยังแสดงบรรทัด) | TC-Q8-DISC-03, TC-Q9-DOC-02 | U/I | Y | เดียวกัน |
| ECP-042 AC3 (Superseded พิมพ์ได้+ลายน้ำ) | TC-Q9-DOC-03 | I | Y | `invoiceDocument.spec.ts` |
| ECP-042 AC4 (ลูกค้า/บริษัทไม่มี tax_id→block) | TC-Q9-DOC-04/05 | I | Y | เดียวกัน |
| ECP-042 AC5 (ยอด=0→VAT/total=0+ศูนย์บาทถ้วน) | TC-Q8-BAHT-01, TC-Q8-DISC-04, TC-Q9-DOC-06 | U/I | Y | `thaiBahtText.spec.ts`, `invoiceDiscount.spec.ts`, `invoiceDocument.spec.ts` |
| thaiBahtText ทั่วไป (เอ็ด/ยี่/ล้าน/สตางค์) | TC-Q8-BAHT-01..10 | U | Y | `tests/unit/thaiBahtText.spec.ts` |

### Epic 14: UI Consistency & Responsive (ใหม่ทั้งหมด, scope-down)

| AC | TC ID | Level | Auto | ไฟล์ |
|---|---|---|---|---|
| ECP-043 AC1 (design token เดียวกัน 8 หน้า) | — | M | N | **ไม่ automate ได้จริง** — เป็นการตัดสินเชิงสายตา (สี/spacing สม่ำเสมอ) ดู `uat-print-responsive-script.md` ส่วนที่ 3; automate ได้แค่ "ไม่มี horizontal overflow" (ดู ECP-044 แถวถัดไป) ซึ่งเป็นคนละมิติจาก "ดูสม่ำเสมอ/เป็นมืออาชีพ" |
| ECP-043 AC2/AC3 (empty/error state เดียวกัน) | — | M | N | เดียวกัน — เชิงสายตา |
| ECP-043 AC4 (หน้านอกขอบเขตไม่ถูกแตะ) | — | M | P | ตรวจได้บางส่วนด้วย git diff/regression run ของหน้าอื่น (ถ้ามี e2e เดิมของหน้านั้นยัง fail ก็แปลว่าโดนแตะ) แต่ "ไม่ได้เปลี่ยนแปลงเชิงสายตา" ยังต้องอาศัยคนตรวจ |
| ECP-044 AC1 (desktop ≥1366 ไม่ล้น) | TC-Q11-RESP-DESKTOP | E | Y | `tests/e2e/responsiveGate2.spec.ts` |
| ECP-044 AC2 (tablet portrait/landscape ไม่ scroll แนวนอน) | TC-Q11-RESP-TABLET-* | E | Y | เดียวกัน |
| ECP-044 AC3 (หมุนจอ ฟอร์มไม่หาย) | TC-Q11-RESP-ROTATE | E | Y | เดียวกัน |
| ECP-044 AC4 (<768 minimum safety) | TC-Q11-RESP-MINSAFETY | E | Y | เดียวกัน (ยืนยันแค่ "ไม่พังทั้งหน้า" ตาม AC4 เอง ไม่ใช่ full fidelity) |
| ใช้งานสะดวกด้วยนิ้วจริงบนแท็บเล็ตจริง | — | M | N | **ไม่ automate ได้จริง** — ต้องใช้มือกดจริง ดู `uat-print-responsive-script.md` ส่วนที่ 2 |

## 10.2 สรุป Automation Coverage — Gate 2 Round 2

- **AC ใหม่/แก้ทั้งหมดรอบนี้**: 44 stories/14 epics ตามที่ dispatcher ระบุ — เฉพาะ AC ที่เกี่ยวข้องกับ 10 feedback
  ของปอนด์โดยตรง (ECP-001/002/004/007/008/013/014/017/020 ที่แก้ + ECP-039/040/041/042/043/044 ใหม่) นับได้
  **~55 AC** ที่ตารางด้านบนครอบคลุม (นับรวม N/A ของ epic อื่นที่ไม่ถูกแตะรอบนี้ — Dashboard/Onboarding/Audit
  ยังคงอยู่ที่สถานะเดิมจาก §2/§9 ไม่มีอะไรเปลี่ยน)
- **Automatable Y**: ~44/55 AC (~80%) — มี TC ที่เขียน spec ไว้แล้วรอบนี้ (unit 6 ไฟล์ใหม่, integration 9 ไฟล์ใหม่,
  e2e 2 ไฟล์ใหม่)
- **Automatable P (partial)**: ~7/55 AC (~13%) — automate ได้ "ข้อมูล/behavior ครบ" แต่ "เชิงสายตา/สุนทรียะ"
  ต้องพึ่งมนุษย์ (print fidelity ภาพรวม, BOM confirm-dialog UX, ECP-043 AC4 หน้านอกขอบเขต)
- **Automatable N**: ~4/55 AC (~7%) — เชิงสายตา/สุนทรียะ/สัมผัสจริงล้วนๆ ไม่มีทางออกแบบ assertion ที่มีความหมาย
  ได้เลย (ECP-043 AC1-3, "ใช้งานสะดวกด้วยนิ้วจริง") — ดู §10.1 ตารางสำหรับเหตุผลต่อรายการ
- **สิ่งที่ automate ไม่ได้ทั้งหมด → ส่งไปที่ `uat-print-responsive-script.md`** (manual script ภาษาไทยสำหรับปอนด์
  ใช้ตอน Gate 2 ตามที่ tasks.md Q11 ระบุ)

## 10.3 ไฟล์ที่เขียนรอบนี้ (Gate 2 Round 2, ทั้งหมดใหม่ ไม่ทับของเดิม)

**Unit (Q8, 6 ไฟล์)**: `tests/unit/thaiBahtText.spec.ts`, `tests/unit/invoiceDiscount.spec.ts`,
`tests/unit/traceAutoDetect.spec.ts`, `tests/unit/fifoAllocation.spec.ts`,
`tests/unit/bomValidation.spec.ts`, `tests/unit/taxIdValidation.spec.ts`

**Integration (Q9, 9 ไฟล์)**: `tests/integration/customerTaxFields.spec.ts`, `tests/integration/bom.spec.ts`,
`tests/integration/productionMaterialPlan.spec.ts`, `tests/integration/traceAutoDetect.spec.ts`,
`tests/integration/invoiceDiscountDetail.spec.ts`, `tests/integration/companyProfile.spec.ts`,
`tests/integration/invoiceDocument.spec.ts`, `tests/integration/goodsReceiptSupplier.spec.ts`,
`tests/integration/rbacGate2.spec.ts`

**E2E (Q10/Q11, 2 ไฟล์)**: `tests/e2e/gate2RegressionGuard.spec.ts` (regression guard 5 defect A-E
+ BOM smoke flow), `tests/e2e/responsiveGate2.spec.ts` (desktop/tablet/rotate/min-safety)

**Manual (Q11, 1 ไฟล์)**: `docs/test-plans/erp-core-prototype/uat-print-responsive-script.md`

## 10.4 Gap ที่รู้ตัวแล้ว แต่ยังไม่ได้เขียนรอบนี้ (ระบุไว้ตรงๆ ตาม exit-gate ของ Phase 1)

1. **ECP-004 AC5** (ลบบรรทัดหลัง PO Confirmed→block ที่ฝั่ง BE) — ยังไม่มี integration test เจาะจง
   (แค่ e2e regression guard คลุม AC1/AC2) ต้องเพิ่มก่อน verify ปิดรอบ
2. **ECP-039 AC3** ส่วน "ยืนยันก่อนลบ" (confirm dialog UX) — integration ครอบ business-rule เต็ม แต่ dialog
   UX เองยังไม่มี e2e เจาะจง
3. **customerTaxFields.spec.ts TC-Q9-TAX-04** (snapshot proof เต็มรูป) — เขียนเป็นโครง placeholder ไว้
   เพราะต้องรอ E30/E32 เชื่อมกันก่อนถึงจะมี invoice จริงให้ snapshot; proof เต็มรูปจริงๆ อยู่ที่
   `invoiceDocument.spec.ts` TC-Q9-DOC-07 แทน (คนละมุมของเรื่องเดียวกัน — ครอบคลุมแล้ว ไม่ใช่ gap ซ้ำซ้อน)
4. ทุกจุดที่มี comment `TODO(verify)`/`CONTRACT ASSUMPTION` ในไฟล์ spec ใหม่ทั้ง 17 ไฟล์ — field names/endpoint
   paths/response shape ทั้งหมดเขียนจาก architecture.md §13 + ADR-009 เท่านั้น (E22-E33 ยังไม่เสร็จตอนเขียน)
   **ต้อง reconcile ทุกจุดตอน verify จริง** เหมือนที่เคยทำกับ DEF-08 รอบก่อน — คาดว่าจะมี field-name/
   endpoint-path mismatch หลายจุด (เป็นเรื่องปกติของ parallel-phase work ไม่ใช่ข้อผิดพลาด)

## 10.5 Exit Gate ของเอกสารนี้ (self-check, Gate 2 Round 2 — Phase 1)

- [x] ทุก AC ใหม่/แก้ (ECP-001/002/004/007/008/013/014/017/020/039/040/041/042/043/044) มี TC ระบุ level +
      automatable ครบ (§10.1)
- [x] Automation coverage ระบุ % พร้อมเหตุผลของส่วนที่เหลือ (§10.2)
- [x] เคสเด่นที่ dispatcher กำชับ (PO line display+delete, stock search, trace L-SEED-1, production
      auto-calc+FIFO+re-validate, invoice detail view, ใบกำกับภาษี thaiBahtText/discount/snapshot,
      customer tax_id validate, BOM CRUD+permission) — **มี spec ครบทุกเคสตามรายการ**
- [x] Manual test script (Q11) เขียนเสร็จเป็นภาษาไทยพร้อมให้ปอนด์ใช้ (`uat-print-responsive-script.md`)
- [x] Gap ที่รู้ตัวแล้วแต่ยังไม่เขียน ระบุไว้ตรงๆ ไม่ซ่อน (§10.4)
- [ ] **รอ Engineer ส่งมอบ E22–E33 ก่อนจึงจะรัน spec เหล่านี้ได้จริง** — เช่นเดียวกับทุกรอบก่อนหน้า
      (parallel-phase policy, ไม่ใช่ blocking item ของเอกสารนี้เอง)
