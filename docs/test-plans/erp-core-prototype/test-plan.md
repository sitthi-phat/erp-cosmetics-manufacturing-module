...
**Default ที่ QA ตั้ง (ต้องปอนด์ยืนยัน — ดู `questions_for_pond`):**
- ผู้ทดสอบ **1 คนต่อ role × 7 roles = 7 คน** (ตรงตามถ้อยคำ AC1 เป๊ะ: "ผู้ใช้ทดสอบใหม่ 1 คนต่อ role")
- **ข้อสังเกตเชิงสถิติ (ความกังขาของ QA)**: sample size = 1 ต่อ role ทำให้ผลลัพธ์ "≥80% ของ scenario"
  วัดได้แค่ระดับ scenario ต่อคน (เช่น คนเดียวทำ 5 scenario จบ 4/5 = 80%) ไม่ใช่ระดับ population ของ role
  นั้นจริง — ถ้าต้องการความมั่นใจว่า 80% เป็นตัวแทนของ role จริง (ไม่ใช่ noise จากคนเดียวที่บังเอิญเก่ง/ไม่เก่ง)
  แนะนำอย่างน้อย 2–3 คนต่อ role เป็นทางเลือกสำรอง — ใส่เป็นคำถามให้ปอนด์เลือก ไม่ใช่ QA ตัดสินใจเอง
...
- [ ] จำนวนผู้ทดสอบ UAT ต่อ role — **รอปอนด์ยืนยัน** (ดู `questions_for_pond`, §5) — ไม่ block เริ่มโค้ด Engineer
      แต่ block การ "เริ่มนัดหมาย UAT จริง" เท่านั้น
