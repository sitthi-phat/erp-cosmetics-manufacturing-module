...
## สรุปภาษาไทย
Prototype (prototype-v1) มี feature ครบ แต่ปอนด์ไม่อนุมัติเพราะ "ไม่ professional / ใช้ยาก / อ่านไม่ออก" v2 ("**ESSENCE Hub System**") กลับด้าน: **ล็อกหน้าตาให้ผ่านก่อนเขียนโค้ด** เน้น **"ทุกสถานะต้องต่อเนื่องกันทั้งระบบ ห้ามหลุด journey"**. Gate 1 ผ่าน 3 รอบยังไม่อนุมัติ — รอบ 3 มี 4 ประเด็นที่ปอนด์สั่ง PO วิเคราะห์ (Dashboard date filter, Goods Receipt multi-line, QC ราย line item, Batch lifecycle) + งาน UX — **ปอนด์ตอบครบแล้ว** รวม **เลข Batch = `B-{PO}-{line}-{run}`** + **rework loop ที่ต้องเห็นชัด**. ข้อวิเคราะห์/คำตอบอยู่ใน `status-journeys.md` §3.1–§3.2, §5, §11–§13
...
## 2.5 การแก้ flow รอบ 3 (`pond-gate1-r3-feedback.md`, 2026-07-08) — ปอนด์ตอบครบแล้ว
**🔍PO 4 ประเด็น (รายละเอียดเต็ม `status-journeys.md`):**
1. **Dashboard date filter (§12):** ✅ preset วันนี้/สัปดาห์นี้/เดือนนี้(default)/กำหนดเอง + **เลือกอิสระ เดือน/ปี/date range**; แยก **event** (PO สร้างในช่วง, ค้างชำระครบกำหนดในช่วง, ห่างหาย=กลายเป็น Inactive, Follow-up=ตั้งในช่วง) vs **state/activity** (ลูกค้าประจำ=มี order ในช่วง; คิวผลิต=ตอนนี้) + **caption ต่อ tile**
2. **Goods Receipt multi-line (§5):** ✅ header + หลาย line (วัตถุดิบ×จำนวน×ราคา×lot gen×อ้าง PR รายบรรทัด); **1 GR อ้างหลาย PR**; **รับไม่ครบ → เสนอสร้าง PR ใหม่เฉพาะที่ขาด + user review ก่อน (ไม่ auto)**
3. **QC ราย line item (§3.2):** ✅ **ตีกลับเฉพาะ line/Batch ที่ไม่ผ่าน** (line อื่นเดินต่อ) + **rework loop ชัดเจน** (badge Rework + feedback + gen Batch run ใหม่ + กลับคิว QC ลื่น) · **QC ตัดสินที่หน้า QC เท่านั้น** (หน้าผลิตไม่มีปุ่ม "QC ไม่ผ่าน")
4. **Batch lifecycle (§3.1):** ✅ gen Batch ตอน "เริ่มผลิต", 1 line = 1 Batch, **เลข Batch = `B-{PO}-{line}-{run}`** (ปอนด์เลือกตามเหตุผล GMP — เช่น B-PO-2026-0012-2-1, ผลิตซ้ำ → run+1); Batch ผูก PO/line/Lot; GMP chain Lot→Batch→line→PO→ลูกค้า
5. **BOM ไม่มี supplier active (§11):** ✅ **บล็อกบันทึกสูตรจนกว่ากรอกราคาทุน override**

**งาน UX อื่น (รอบ 3):** PO status-change UI + trace · หน้า create จริง (เพิ่มลูกค้า/ผู้ติดต่อ/supplier/PR/BOM/รอบจัดส่ง) · supplier layout ใหม่ (ไม่เอา panel ขวา) · shipment round data (คนขับ/เบอร์/Route/ประเภทรถ เก๋ง/motorcycle/กระบะ/10 ล้อ) · ตรวจ label ซ้ำทุกหน้า (ปอนด์เจอ "Dashboard" 3 จุด) · dropdown search ได้ทุกจุด · test data สมจริง
> รายการสั่งงาน UX/UI รอบ 4 (P0/P1/P2) อยู่ `po-mockup-review.md` §8 — **หน้าผลิต/QC/po-detail ต้องแสดง Batch format ใหม่ + rework loop (§3.2 ก–ง)**
...
- [ ] mockup coverage 100% ทุกหน้า + ทุกสถานะ (ลูกค้า 6 สถานะ, dashboard 7 แผนก drill-down+auto-refresh+date-filter, Shipment/DN 2 ชั้น, Batch/QC ราย line + rework loop)
...
- [ ] Goods Receipt multi-line + อ้าง PR หลายใบ · QC ราย line + **Batch `B-{PO}-{line}-{run}` + rework loop** เห็นชัด
- [ ] Dashboard date filter (preset + เลือกอิสระ + caption ต่อ tile) · shipment round data ครบ (คนขับ/เบอร์/route/รถ)
...
**Tier 1 — หัวใจ:** PO create+summary · Production (flow ใหม่ + Batch/rework) · Invoice+ใบกำกับภาษีไทย · Stock · Customer (6 สถานะ)
...
- **Status-journey continuity:** ทุกการเปลี่ยนสถานะต่อเนื่องข้าม module ตาม `status-journeys.md` + **trace เสมอ** (รวม cancel/reopen, QC rework, Hold, Follow-up, PO status-change) + comment (บังคับในจุดที่ระบุ)
- **Responsive ทุกหน้า (Must)**
- **GMP traceability:** ไล่ **Lot → Batch → line → PO → ลูกค้า**; **Batch สร้างตอนเริ่มผลิต ผูก PO/line/Lot, เลข `B-{PO}-{line}-{run}`**; audit ระดับ field; archive text file เฉพาะ Super User
- **ใบกำกับภาษีไทย:** LOGO, ผู้ออก+เลขผู้เสียภาษี 13 หลัก, ลูกค้า, เลขที่/วันที่/เครดิต, ตารางรายการ, สรุปยอด (subtotal, **discount**, **VAT7% + effective date (ยึด invoice date)**, grand total, **ตัวหนังสือไทย**), **ลายเซ็น 2 ช่อง**, ตรายาง
...
- **Dashboard รายแผนก 7 แผนก**: refresh + auto-refresh 15s (default) ค้าง view; **date filter (วันนี้/สัปดาห์/เดือน default/กำหนดเอง + เลือกอิสระ เดือน/ปี/range) มีผลทุก tile + caption ต่อ tile (§12)**; KPI tile drill-down → list + pagination; Sale tile "ต้องติดตาม"
...
- **PO**: summary, suggest + เช็ควัตถุดิบ (ขาด=เตือนไม่บล็อก + auto PR), ขาย BOM+วัตถุดิบ, แก้จำนวน+ราคา (0 ได้), search 3 วันที่, cancel+reopen (คงเลขเดิม), **UI เปลี่ยนสถานะ PO + trace**, **ภาพรวมทุก line/Batch (ผ่าน/รอ QC/Rework)**, 2 ราง
- **Stock**: เพิ่มวัตถุดิบ+UOM, เกณฑ์ใกล้หมด (ไม่มี default), ราคา 0 ได้, **หน้า Goods Receipt multi-line** (header + หลาย line×lot gen×อ้าง PR หลายใบ; รับไม่ครบ → เสนอ PR ใหม่เฉพาะที่ขาด user review ก่อน)
...
- **BOM builder**: **หน้าสร้างสูตรใหม่ กดได้จริง**, ราคาทุน = max active supplier + แก้ทับได้ + snapshot + badge เตือน, **ไม่มี supplier active → บล็อกจนกรอก override**, ราคาขาย mandatory
- **Production**: flow รับงาน→กำลังผลิต→รอ QC→พร้อมส่งมอบ; **gen Batch ตอนเริ่มผลิต (`B-{PO}-{line}-{run}`, 1 line=1 Batch)**; **หน้าผลิตไม่มีปุ่ม "QC ไม่ผ่าน"**; **rework loop (badge Rework + feedback + ผลิตซ้ำ gen run ใหม่ + line อื่นเดินต่อ)**; Hold→แก้ PO→ผลิตต่อ; Potential Delay
- **QC**: incoming inspection + **QC ราย line item** (ไม่ผ่าน→ตีกลับเฉพาะ line+feedback; คิว QC เห็น rework เป็นรายการใหม่+ประวัติ run); QC ตัดสินที่หน้านี้เท่านั้น; เห็น PO↔Batch↔Lot
- **Purchase Request**: **หน้าสร้างคำขอใหม่จริง**, ของเข้าครบ auto จาก GR, รับทราบ/ปิดคำขอ manual, partial receipt (เสนอ PR ใหม่ user ยืนยัน)
...
- **Invoice**: ออกได้ตั้งแต่ PO=Confirmed + โชว์ PO status, overdue (เครดิตระดับลูกค้า/ต่อใบ), versioning, ใบกำกับภาษีไทย (VAT ตาม effective date ณ invoice date)
...
- [ ] **Gate 1 ผ่าน:** mockup ครบ 100% ทุกหน้า/ทุกสถานะ (flow รอบ2–3 + 6 สถานะ + Shipment/DN + Batch/QC ราย line + rework loop + GR multi-line) + หน้า create จริง + cross-module continuity + responsive + Settings 5 หน้าจอ — ปอนด์อนุมัติ
- [ ] **Engineer implement** ครบ scope §6 + ทุก status journey + กติกา §2.2–§2.5 โดยหน้าตาตรง mockup + responsive
- [ ] **QA: Functional = 0 defect ทุกระดับ** + ทดสอบทุก state transition & cross-module notify + GMP chain + rework loop
...
- GMP / full traceability (Lot→Batch→line→PO, Batch `B-{PO}-{line}-{run}`, audit ระดับ field) + status-journey continuity + responsive เป็นข้อบังคับแฝงทุก feature
...
- **เอนทิตี/field ใหม่:** **Batch (สร้างตอนเริ่มผลิต, 1 line=1 Batch, เลข `B-{PO}-{line}-{run}`, run เพิ่มตอน rework, ผูก PO/line/Lot)**, **Goods Receipt multi-line (header + lines, lot gen รายบรรทัด, อ้าง PR หลายใบ, partial→PR ใหม่ user review)**, **Shipment (รอบจัดส่ง: คนขับ/เบอร์/route/ประเภทรถ)**, Delivery Note (1 ใบ=1 PO), Purchase Request (สร้างตรง + partial receipt), Supplier (+active/inactive + **Supplier×Material price matrix**), Customer Contact (หลายรายการ, หน้าเพิ่มจริง), **Customer สถานะที่ 6 "Follow-up" + comment**, ใบคืนของ (Return), UOM, ราคาซื้อ/ขาย/ทุน แยก field (0 ได้), **BOM cost snapshot + badge ล้าสมัย + block เมื่อไม่มี active supplier**, VAT effective date (ยึด invoice date), เครดิตระดับลูกค้า+ต่อใบ, **field-level audit**, **PO cancel/reopen (คงเลขเดิม) + PO status-change UI + line/Batch overview**, Postpone flag + date, **Vehicle type list (config ได้)**
...
- **รอบ 3 (r3-analysis): ปอนด์ตอบครบทุกข้อแล้ว** — QC ตีกลับเฉพาะ line + rework loop; **เลข Batch `B-{PO}-{line}-{run}`**; dashboard default เดือนนี้+เลือกอิสระ; GR อ้างหลาย PR + partial→PR ใหม่ user review; BOM block เมื่อไม่มี active supplier → sync แล้วใน `status-journeys.md` §3.1/§3.2/§5/§11/§12/§13 · **ไม่มีคำถามค้าง**
