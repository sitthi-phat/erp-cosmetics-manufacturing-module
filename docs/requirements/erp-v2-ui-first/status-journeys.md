...
ที่มา: `pond-gate1-feedback.md` (ร1) + คำตอบ 6 ข้อ + `pond-gate1-r2-feedback.md` (ร2) + คำตอบ 5 ข้อ + `pond-gate1-r3-feedback.md` (ร3) + คำตอบ r3 ครบ + **`entity-status-map.md` (r4/r4.1 — ความจริงหลักเรื่อง entity/สถานะ/ใครเปลี่ยน/cascade รวมนิยาม PRD + negative stock)** + Notification/deep-link

## สรุปภาษาไทย
"แผนที่สถานะ" ของทั้งระบบ — ทุกสถานะต้องต่อเนื่องข้าม module ห้ามหลุด journey. **นิยาม entity/สถานะ/cascade ฉบับเต็ม + PRD (ใบสั่งผลิต) อยู่ที่ `entity-status-map.md` (ความจริงหลัก) เอกสารนี้อ้างอิงและสอดคล้องกับมัน**. สรุป r4.1 (ปอนด์ปรับ): **PO ยืนยัน → แต่ละ line เข้าคิวผลิต "รอรับงาน" (ยังไม่เกิด PRD) → ฝ่ายผลิตกด "รับงาน" เอง = สร้าง PRD → กด "เริ่มผลิต" = gen Batch** · **1 PO : N PRD (N=line) : M Batch (M≥N, +1/rework)** · QC ตัดสินราย Batch ที่หน้า QC เท่านั้น · **วัตถุดิบขาดไม่บล็อก + ผลิตตัด stock ติดลบได้ (GR บวกกลับ + โชว์ชัด)** · เลข `B-{PO}-{line}-{run}`, `PRD-{YYYYMM}-{NNNNNN}` · ก่อนหน้า: ลูกค้า 6 สถานะ, Shipment→DN (DN=1 PO), GR multi-line/partial, dashboard filter, BOM block/snapshot

**หลักการร่วม (ทุกสาย):**
1. **ทุกการเปลี่ยนสถานะมี trace เสมอ** (ใคร/จากอะไร→เป็นอะไร/เมื่อไหร่/เหตุผล) — รวม cancel/reopen + การตัด stock ติดลบ
...
**ลบลูกค้า:** ไม่มี hard delete — ดู `deletion-policy.md` (soft delete + ห้ามลบถ้ามี PO active)
...
stateDiagram-v2
    ...
    Confirmed --> InProduction: แต่ละ line เข้าคิว "รอรับงาน" → ฝ่ายผลิตกด "รับงาน"(gen PRD) → "เริ่มผลิต"
...
```
- **Cancel ได้ทุก case** (บังคับ comment) · **Cancelled → Draft** คงเลข PO เดิม + trace (PO ไม่มี hard delete — ดู `deletion-policy.md`)
- **UI เปลี่ยนสถานะ PO ชัดเจน** ที่ po-detail (ปุ่ม/กล่อง + เหตุผล + trace). manual force เฉพาะ Admin-bit (RUCDAA) + trace
- **po-detail แสดงภาพรวมทุก line item + PRD/Batch/สถานะราย line** (line ไหน รอรับงาน/rework/ไปต่อ/ผ่าน QC) — เห็นทั้งใบในที่เดียว
...
> **นิยามเต็ม + สถานะราย entity + cascade + negative stock ดู `entity-status-map.md` (ความจริงหลัก)**
**PRD (ใบสั่งผลิต) = 1 ใบต่อ 1 line item · สร้างเมื่อฝ่ายผลิตกด "รับงาน" (ไม่ auto ตอน Confirm — ปอนด์ปรับ r4.1) · 1 PRD มีได้หลาย Batch (run) เมื่อ rework**
**ก่อนเกิด PRD:** PO Confirmed → แต่ละ line เป็น **"คิวรอรับงาน (Awaiting Acceptance)"** ยังไม่มีเลข PRD · ฝ่ายผลิตกด "รับงาน" จึง gen PRD
สถานะ **PRD**: *(pre)* `รอรับงาน` → `รับงาน` → `กำลังผลิต` → `รอ QC` → `พร้อมส่งมอบ` + `พักงาน (Hold)` + `Rework` + overlay `เสี่ยงล่าช้า`
...
**วัตถุดิบขาดไม่บล็อก:** รับงาน/เริ่มผลิตได้แม้ stock ไม่พอ → เตือน + ตัด stock ติดลบได้ (§3.3)

```mermaid
stateDiagram-v2
    [*] --> AwaitingAcceptance: PO Confirmed -> line เข้าคิว "รอรับงาน" (ยังไม่มีเลข PRD)
    AwaitingAcceptance --> Received: Production กด "รับงาน" -> gen PRD-{YYYYMM}-{NNNNNN}
    Received --> InProgress: กด "เริ่มผลิต" -> gen Batch B-{PO}-{line}-{run} + ตัด stock (ติดลบได้)
...
```
- ตัวเปลี่ยนสถานะที่หน้าผลิต = **รับงาน** / เริ่มผลิต / ส่งตรวจ QC / Hold(+raise Sale) / **ผลิตซ้ำ (เมื่อ line ถูก rework)** — **ไม่มี "QC ไม่ผ่าน"**
...
- **PRD สร้างเมื่อ:** **ฝ่ายผลิตกด "รับงาน"** (ปอนด์ปรับ r4.1 — ไม่ auto ตอน Confirm) · **1 line = 1 PRD** → 1 PO N line = **N PRD** · เลข PRD ออกตอนรับงาน (gapless ต่อเดือน)
...
- **(ง) line อื่นใน PO เดินต่อไม่สะดุด** + **po-detail เห็นภาพรวมทุก line/PRD/Batch** (สถานะราย line: รอรับงาน/ผ่าน/รอ QC/Rework)
- ทุกขั้นมี **trace** (ใคร/เมื่อไหร่/feedback/run)

| Transition | หน้า | comment | สะกิดข้าม module |
|---|---|---|---|
| PO Confirmed → line เข้าคิว "รอรับงาน" | ระบบ (auto) | — | โผล่คิวฝ่ายผลิต (ยังไม่มีเลข PRD) |
| รอรับงาน → รับงาน (gen PRD) | **Production ("รับงาน")** | — | ออกเลข PRD `PRD-{YYYYMM}-{NNNNNN}` |
| Received → InProgress (gen Batch run 1) | Production ("เริ่มผลิต") | — | สร้าง Batch `B-{PO}-{line}-1` ผูก PO/line/Lot + ตัด stock (ติดลบได้) |
...
### 3.3 Negative Stock (ปอนด์ตอบ r4.1 — วัตถุดิบขาดไม่หยุดสายผลิต)
- **ไม่บล็อก:** รับงาน/เริ่มผลิตได้แม้ stock วัตถุดิบไม่พอ · Batch เริ่มผลิต **ตัด stock ได้จนติดลบ** — บันทึก trace (Batch/วัตถุดิบ/ปริมาณติดลบ/เมื่อไหร่/ใคร)
- **เหตุผล:** ของจริงอาจมาถึงแล้วแต่ยังไม่ได้ทำใบรับในระบบ — ห้ามหยุดผลิต
- **GR บวกกลับ:** เมื่อ Goods Receipt วัตถุดิบตัวติดลบ → stock บวกกลับ (ชดเชยติดลบก่อน) + **หน้า GR แจ้ง "การรับนี้ชดเชยยอดติดลบ X หน่วย"**
- **แสดงผล (UX/UI):** stock.html ยอดติดลบ = สีแดง + badge "ติดลบ (รอรับเข้า)"; production เตือนตอนรับงาน/เริ่มผลิต; goods-receipt กล่อง notice ชดเชย
...
- **★ ชดเชย negative stock:** ถ้าวัตถุดิบที่รับเคยติดลบ (ผลิตไปก่อน) → **แจ้ง "การรับนี้ชดเชยยอดติดลบ X หน่วย"** ก่อนยืนยัน แล้วบวก stock กลับ (§3.3)
...
| C4 | Goods Receipt รับของ (ผูก lot รายบรรทัด) | PR → Fulfilled อัตโนมัติ (รับไม่ครบ = เสนอ PR ใหม่ user ยืนยัน); Stock เพิ่ม (lot รอ QC) + ชดเชยยอดติดลบ (โชว์ notice) |
| C5 | **PO Confirmed** | **แต่ละ line เข้าคิวผลิต "รอรับงาน"** (ยังไม่เกิด PRD) |
| C5a | Production กด "รับงาน" | **gen PRD** `PRD-{YYYYMM}-{NNNNNN}` (สถานะ รับงาน) |
| C5b | Production "เริ่มผลิต" (PRD) | gen Batch `B-{PO}-{line}-{run}` (ผูก PO/line/Lot); PRD/Batch = กำลังผลิต; **ตัด stock (ติดลบได้)** |
...
| C13 | PO Cancelled → Draft (reopen, คงเลข) | คิว/PRD/Batch ที่เกิดถูกยกเลิกตาม + trace; รับงานต่อได้ |
...
- **Delete (D bit) + soft delete:** การลบเป็น soft delete เท่านั้น (flag "deleted") — สิทธิ์/กติกาต่อ entity ดู `deletion-policy.md`
...
- **r4 (lifecycle PRD): ตอบครบแล้ว ✅ (ปอนด์ r4.1)** — (1) PRD **ไม่ auto** ฝ่ายผลิตกด "รับงาน" เอง (เพิ่มคิว "รอรับงาน") · (2) 1 line=1 PRD · (3) วัตถุดิบขาด**ไม่บล็อก** + **stock ติดลบได้** (§3.3) — ดู `entity-status-map.md`
- **r4 (Deletion Policy): ร่างแล้ว** — คำถามถึงปอนด์อยู่ใน `deletion-policy.md` (soft delete ทุก entity + กติกาต่อ entity)
