...
slug: `erp-v2-ui-first` · per-module canonical · PO · 2026-07-29 (**+ Route/DN rename 2026-07-30**) · **AUTHORITATIVE convention (documented ONCE, referenced by every create screen that issues a gapless document number)**
...
| **★★★ DN + Route (จัดส่ง) — r11** | **`RT-{YYYYMMDD}-{NNNN}` (★ เดิม `SHP-…` — Q1)** + `DN-{YYYYMMDD}-{NNNNN}` (ราย order) | `shipping.md` §3/§5 · `delivery-note.md` §3 | กด **สร้าง Route (shipping create-round)** | คนขับ/route/ประเภทรถ · จำนวน order · เลข RT + **ทุก DN ที่ gen (PO/SO ใบไหนได้ DN ใด)** (NS7) | เปิด/พิมพ์ DN | **ขยาย** · หลาย DN ต่อรอบ → NS7 · **★ Route = ชื่อใหม่ของ Shipment** |
...
- **★ r11: รอบจัดส่งเปลี่ยนชื่อ Shipment → Route, เลข `RT-…` (★ Q1 vs SHP — PO เสนอ RT แทน SHP).** RT + ทุก DN ในรอบ ออกพร้อมกันตอน "สร้างรอบ" (NS7). พฤติกรรม number-on-save คงเดิมทุกประการ (เปลี่ยนแค่ prefix/ชื่อ).
...
- **★★★ UPDATED (2026-07-30 — Route/DN r11):** "DN + Shipment (`SHP-…`)" → **"DN + Route (`RT-…`)"** (★ Q1 vs SHP — PO เสนอ RT แทน SHP); trigger = "สร้าง Route"; popup NS7 แสดง RT + ทุก DN + ระบุ PO/SO ใบไหนได้ DN ใด. §4 doc list · §5 · §6 · summary. พฤติกรรมคงเดิม (เปลี่ยน prefix/ชื่อ).
