...
กฎอ้างอิง: entity-status-map §1.2/§1.3 (2 ราง) · stock-reservation (จอง→ตัดจริง Option A) · D3 · D13 · D18 · README §3

## สรุปภาษาไทย
ใบสั่งซื้อ OEM (รับจ้างผลิต, made-to-order). Create ("เปิดใบสั่งซื้อใหม่") เพิ่ม **customer search dropdown** (โชว์สถานะ+credit term, ดู detail แบบ modal แล้วกลับไม่เสีย state). line = BOM/วัตถุดิบตรง (RM-direct ยังผ่านขั้นผลิต D3). ยืนยัน PO = จองวัตถุดิบ (Reserve); ขาด → เตือน + auto PR (ไม่บล็อก). รองรับ **origin ref "created from QT-…"** (สร้างตรงหรือมาจาก Convert-to-PO). ผลิตเกิน → surplus เข้า FG ตอน "พร้อมส่ง" (D13). 2 ราง: fulfilment + billing (credit term 30/60/90 default 60).
...
| ลูกค้า | ref customer | editable (dropdown G4) | โชว์สถานะ + credit term |
