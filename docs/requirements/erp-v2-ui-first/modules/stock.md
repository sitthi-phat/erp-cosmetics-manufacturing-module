...
- **FG เข้าคลังเมื่อ:** (1) produce-to-stock Batch **QC ผ่าน → กด "พร้อมส่ง"** → `FG-in (+)` (production.md §4) · (2) OEM **surplus** ตอน "พร้อมส่ง" → `surplus (+)` (คง Batch identity ของตัวเอง ผูก OEM Batch/PRD/PO).
- **1 BOM = 1 FG แบบ 1:1 · FG มีรหัส = รหัส BOM (แชร์รหัสเดียว)** → FG **ค้นได้ทั้งชื่อและรหัส** ในแท็บ FG (search dropdown). **★ ที่มารหัส = ผู้ใช้ตั้งเองตอนสร้าง BOM + unique + ล็อกหลังสร้าง (D11 v2)**; รายละเอียด = `bom.md` §3/§5. (OEM BOM ก็มี FG identity + รหัส เช่นกัน — ปกติยอด 0 ยกเว้น surplus D13.)
...
- **★ delta (ปอนด์สั่ง 2026-07-29 — Production review):** เดิม Adjust (+) เป็น **RM-only** (ไม่อ้าง Lot). ปอนด์กำหนดใหม่: **Adjust ต้องอ้าง Lot เสมอ — เลือก lot ที่ต้องการ, หรือถ้าไม่มี/จำไม่ได้ ให้เลือก "FIFO"** — สมมาตรกับ Loss. เพื่อคง genealogy ราย lot ครบทุก movement.
...
> **หมายเหตุความสมมาตร:** ปอนด์สั่งแยก 2 action (Loss −/Adjust +) เฉพาะ **แท็บ RM** (ทั้งคู่ตอนนี้อ้าง Lot/FIFO); ฝั่ง **FG** ยังคง adjust ± ตรง + loss ตามเดิม. UX/UI ทำตามนี้.
...
> **★ delta 2026-07-29 (ปอนด์):** (a) เดิม `adjust (±)` → แยกเป็น **`loss (−)`** + **`adjust (+)`**. (b) **Production review:** **`adjust (+)` เพิ่ม Lot ref (เลือก lot หรือ "FIFO")** เหมือน `loss (−)`. **(c) ★ QC + GR/Stock flow review: `GR (+)` เกิดตอน QC ตรวจรับ "ผ่าน" เท่านั้น (ไม่ใช่ตอนบันทึก GR)** — ชดเชยติดลบ + FIFO retro-link ย้ายมาที่ QC pass; QC ไม่ผ่าน = ไม่มี `GR (+)` movement. CONSUME ระบุชัด "เลือกเฉพาะ lot ที่มี stock; หลาย lot = FIFO" (production.md §5d). **(d) ★ Return module: `return (−)` source ผูก Lot + RM + Supplier + เลขใบคืน (เลือก RM ในล็อต — 1 lot หลาย RM, `return.md`).**
> **★ delta 2026-07-31 (reconcile M1, ปอนด์):** `return (−)` source token เปลี่ยนจาก "RT" (กำกวมกับ Route) → **เลขใบคืน `RET-…`** (prefix เฉพาะของใบคืน, `return.md`/`numbering-on-save.md` §4). กลไก ledger คงเดิม — เปลี่ยนแค่ label ให้ตรงเลขเอกสารจริง.
...
## 9. Validations
- **RM code: ตอนสร้าง = บังคับ + UNIQUE** (reject ถ้าซ้ำก่อนบันทึก) · **หลังสร้าง = read-only (create-only-lock)** · ชื่อ + หน่วยนับ บังคับ.
...
- **★ แท็บ Good Receipt (RM): 20/หน้า (G1) · ค้น GR/Lot/Supplier/ชื่อ RM/รหัส/★ ช่วงวันที่รับ · filter สถานะ (ผ่าน/ไม่ผ่าน/QC ตรวจสอบ/ยกเลิก) (§2b, G2).**
...
## 11. Cross-links
- reservation → stock-reservation · **★ GR/retro-link/credit ตอน QC ผ่าน → entity-status-map §1.6/§1.8 · `goods-receipt.md` §4/§9 · `qc.md` §4.1** · **★ เลข GR+Lot ออกตอนบันทึก (G8/NS7) → `numbering-on-save.md` · `goods-receipt.md` §5** · **RM master ที่สร้างที่นี่ → อ้างใน `goods-receipt.md` (GR line), `bom.md` (component), `return.md` (RM ในล็อต)** · **FG/รหัส FG + create-only-lock → `bom.md` §5 (1 BOM=1 FG, code)** · FG source + loss/lot-FIFO consume → `production.md` §5/§7 (surplus, loss, FIFO), `so.md` (produce-to-stock FG-in) · **★ `return (−)` = เลขใบคืน `RET-…` → `return.md` · `numbering-on-save.md` §4** · **audit/trace ทุก movement + add-RM + GR status → `traceability.md` §3/§4 · `non-functional.md` AU3** · trace → scope §8.5.
...
- **★ เพิ่ม (2026-07-29 — Return module feedback, ปอนด์):** `return (−)` source ผูก **Lot + RM + Supplier + เลขใบคืน** (เลือก RM ในล็อต — 1 lot หลาย RM); RM master เลือกได้ใน Return (§2/§6/§8, ref `return.md`).
...
- **★ CHANGED (2026-07-29 — Production module review, ปอนด์):** **Adjust (ปรับยอด +) ต้องอ้าง Lot เสมอ (เลือก lot หรือ "FIFO")** (เดิม RM-only) — §5.1/§6/§9/§10.
- **★ NEW/CHANGED (2026-07-29 — QC + GR/Stock flow review, ปอนด์):**
  1. **★ แท็บใหม่ "Good Receipt (RM)"** — ลิสต์ GR ทุกใบ + search (GR/Lot/Supplier/ชื่อ RM/รหัส/ช่วงวันที่รับ) + filter สถานะ (ผ่าน/ไม่ผ่าน/QC ตรวจสอบ/ยกเลิก) + action **ส่งกลับ QC / ยกเลิก** — §2/§2b/§8/§9/§10, ref `goods-receipt.md` §4.
  2. **★ `GR (+)` credit เกิดตอน QC ตรวจรับ "ผ่าน" เท่านั้น** (ไม่ใช่ตอนบันทึก GR) — ชดเชยติดลบ + FIFO retro-link ย้ายมาที่ QC pass; QC ไม่ผ่าน = ไม่มี `GR (+)` — §1/§3/§6/§11, ref `qc.md` §4.1 · `goods-receipt.md` §9 · entity-status-map §1.8.
- **★★ CHANGED (2026-07-31 — reconciliation M1, ปอนด์):** `return (−)` ledger source token **"RT" → "RET"** (เลขใบคืน `RET-…`) เพื่อเลิกความกำกวมกับ Route `RT-…` — §6 (table + delta note) · §11 cross-link · summary. อ้าง `return.md` · `numbering-on-save.md` §4 · `non-functional.md` D-F5. กลไก ledger คงเดิม.
