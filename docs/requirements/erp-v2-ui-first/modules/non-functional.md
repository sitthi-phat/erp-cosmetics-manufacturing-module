...
ที่มา (locked, ไม่ประดิษฐ์ตัวเลข): `brief.md` §5/§8 · ADR-000..009 · `scheduled-jobs.html` (J1–J7) · architecture `index.html`/`db-schema.html` · `entity-status-map.md` §1.6 (negative stock) · `stock-reservation.md` (Option A) · `deletion-policy.md` · scope D1–D18 · README §2/§3 · **`supply-planning.md` §5.1 (Low alerting — J8)** · **`comment-convention.md` (comment field = audited)** · **`stock.md` §3b/§6 (add-RM + loss/adjust movements)** · **`bom.md` §5/§5c (code user-entered-locked + BOM changes/inactivate)** · **`supplier.md` §10 (supplier + price-matrix audit)**

## สรุปภาษาไทย
รวม **ข้อกำหนดที่ไม่ใช่ฟังก์ชัน (NFR)** ทั้งหมดของ ESSENCE Hub ไว้ที่เดียว และอัปเดตให้ครอบ scope ใหม่ (OEM Quotation / Own-Brand SO / Supply Planning / FG). ครอบ: **Performance** (หน้า <2s max 3s, 50 concurrent, >200 PO/วัน + GR>50/วัน + เพิ่ม QT/SO/Supply-Planning เข้า load profile), **Auth/Session** (local+Google, 24h + เตะออก 06:00 ทุกวัน ไม่มีกะดึก, RBAC generic RUCDAA+Admin bit — D14), **Audit** (field-level, retention 1 ปี แล้ว manual purge; **ทุก stock movement — รวมเพิ่มวัตถุดิบใหม่/loss(−)/adjust(+) — มี reason+source + audit + trace (D15)**; **ทุก action ของ QT ก็ถูก audit เต็ม**; **★ ทุกการเปลี่ยน BOM (สร้าง/แก้/ราคาซื้อ/inactivate) และ Supplier (create/edit/active-inactive/price-matrix) ก็ถูก audit เต็ม**; **★ ช่อง `comment` ทุก object ธุรกรรมก็เป็น field ที่ถูก audit เต็ม — แก้ทับได้แต่เก็บประวัติ**), **Backup/Infra** (GCP Cloud Run + Cloud SQL MySQL, backup รายวัน, storage abstraction GCS-ready), **Data/Format** (Asia/Bangkok, พ.ศ. display / UTC store, gapless numbering ต่อปี/เดือนทุกชนิดเอกสารรวม QT-/SO-/PRD/Batch/DN/INV; **★ RM code + BOM/FG code = ผู้ใช้ตั้งเองตอนสร้าง + unique + ล็อกหลังสร้าง (create-only-lock)**, THB, VAT ตาม invoice date), **Scheduled Jobs J1–J8** (เพิ่ม **J8 = Supply Planning low-stock daily digest ~06:00**), **Notification outbox+read-bit** (รวม event ใหม่ **FG→Low** เชิงรุก), **global search**, **responsive**, **soft-delete + reference-guard** (รวม **BOM = inactivate ไม่ hard delete**), **reliability/integrity**. ทุกข้อ derive จากคำตอบที่ปอนด์ล็อกแล้ว — ไม่มีการเดา.
...
| D-F4 | สถานะ/UI | ป้ายสถานะ**ภาษาไทย**เสมอ · ไม่มี enum ดิบ · **dropdown search ได้ทุกจุด: RM ค้นชื่อ+รหัส, FG ค้นชื่อ+รหัส, Lot ค้น dropdown (stock.md §10) · BOM component RM ค้นชื่อ+รหัส (bom.md §3) · Supplier price-matrix RM ค้นชื่อ+รหัส (supplier.md §3) · customer dropdown (G4)** |
...
> **หมายเหตุ:** การแก้ `comment`, stock movement (loss/adjust/add-RM), BOM inactivate/edit, supplier price-matrix = audit event (AU1/AU3) แต่ **ไม่ยิง notification** (เป็นการบันทึกภายใน; ยกเว้น movement ที่ทำให้ FG พลิกเข้า Low → ยิงตาม §6.1) เว้นแต่ปอนด์ระบุเพิ่ม.
...
**ไม่มี open question ค้าง.** คำถามเดิมเรื่อง Supply Planning proactive alert = **ปิดแล้ว (ปอนด์ตัดสิน real-time + J8 digest — §6.1)**. NFR อื่นทั้งหมด derive จากค่าที่ล็อกแล้ว. (comment field audit + stock movement audit + BOM/Supplier change audit = ใช้กลไก field-audit/ledger เดิม — ไม่มี NFR ใหม่ที่ต้องถาม.)

## 13. Cross-links
- Auth/session/RBAC → `settings.md`/`platform.md` · Audit → `traceability.md` · ledger reason/source + add-RM/loss/adjust → `stock.md` §3b/§6 · **BOM code/inactivate + audit → `bom.md` §5/§5c** · **supplier + price-matrix audit → `supplier.md` §10** · negative stock/retro → `goods-receipt.md`/entity-status-map §1.6 · reservation → `so.md`/`po.md`/stock-reservation · soft-delete/void/inactivate → `deletion-policy.md` · **Low alerting → `supply-planning.md` §5.1 + `platform.md` §7/§9** · **QT activity/audit → `quotation.md` §10 + `traceability.md` §4** · **comment field audit → `comment-convention.md` + `traceability.md` §4** · jobs → architecture scheduled-jobs (TL).
...
- **★ เพิ่ม (2026-07-29 — Quotation module review):** AU1 ระบุชัดว่า **ทุก action ของ Quotation (create/edit→version/convert→Confirmed/cancel) ถูก audit + แสดง trace** (§3, cross-ref quotation.md §10 / traceability.md §4).
- **★ REVERTED (2026-07-29 — Pond):** ถอด **"send (Sent) + sent-date"** ออกจาก AU1 QT action list — การส่งใบเสนอราคา = print/share ไม่เปลี่ยนสถานะ/ไม่มี sent-date. sync `quotation.md` §4 + `traceability.md`.
...
- **★ เพิ่ม (2026-07-29 — Stock module 4 review):** AU3 ขยายให้ระบุชัดว่า **ทุก stock movement (เพิ่มวัตถุดิบใหม่/loss −/adjust +/GR/FG-in/surplus/return, RM+FG) ถูก audit + trace + reason/source (D15)** · D-F5 ระบุ **RM code = user-defined unique** vs FG/BOM code · D-F4 dropdown search RM/FG ชื่อ+รหัส. cross-ref `stock.md` §3b/§6, `traceability.md` §4.
- **★ CHANGED/เพิ่ม (2026-07-29 — BOM + Supplier module review, ปอนด์):**
  1. **D-F5:** **RM code + FG/BOM code = ผู้ใช้ตั้งเองตอนสร้าง + unique + create-only-lock (แก้ไม่ได้หลังสร้าง)** — FG/BOM เดิมระบุ "auto"; ปอนด์ปรับ ("สร้างได้ แต่แก้ไขไม่ได้ RM ก็ด้วย") → ที่มาเป็น user-entered, 1:1+shared คงเดิม, เพิ่ม lock. AU3 เพิ่ม RM create-only-lock.
  2. **AU1:** เพิ่ม **BOM changes (create/edit/ราคาซื้อ/inactivate/reactivate)** + **Supplier changes (create/edit/active-inactive/price-matrix)** เป็น field-audit + trace เต็ม (cross-ref bom.md §5c/§9, supplier.md §10, traceability.md §4).
  3. **§10:** เพิ่ม **BOM = inactivate (ไม่ hard delete) + inactive บล็อก QT/PO/SO** เข้า soft-delete baseline (deletion-policy §2.4).
  4. **D-F4:** เพิ่ม BOM component + Supplier price-matrix RM search ค้นชื่อ+รหัส · **§6/§6.1 + J8:** ประเมินเฉพาะ FG **Active** (Inactive ถูกกันออก).
