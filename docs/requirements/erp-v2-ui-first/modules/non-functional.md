...
| D-F4 | สถานะ/UI | ป้ายสถานะ**ภาษาไทย**เสมอ · **dropdown search: RM/FG ชื่อ+รหัส, Lot dropdown (+FIFO) · BOM component · Supplier price-matrix · customer dropdown (G4) · Supply Planning FG · Return RM ในล็อต (G7)** · Settings role/user · Production queue · **★ r10: stock "Good Receipt (RM)" tab ค้น GR/Lot/Supplier/ชื่อ+รหัส RM + วันที่รับ + filter สถานะ · qc ตรวจรับ/ตรวจแบตช์ (sub-tab OEM/Own-Brand)** · **★★★ r11: Route list ค้นคนขับ/username/route-id + ช่วงวันชนิดวัน (สร้าง/ออกไปส่ง) · DN list ค้น คนขับ/username/route-id/PO-SO/วันลูกค้าต้องการรับ + filter สถานะ DN (6)** · **★★★★ r12: trace entity/topic selector + ค้น id/key + ช่วงวันชนิดวัน (date-type) · audit-log ค้น user id/username + ช่วงวัน + filter module** |
...
- **★★★★ r12 — login-failure audit (PO reasonable decision, ไม่ block):** audit บันทึก **ทั้ง login สำเร็จและล้มเหลว + logout** (มาตรฐาน security). ปอนด์ override ได้ (เช่น เก็บเฉพาะสำเร็จ). ไม่มีนโยบาย lockout/rate-limit ระบุ — ถ้าต้องการให้ระบุเพิ่ม (นอกขอบ audit).
...
- **★★★★ เพิ่ม (2026-07-30 — Audit non-read+login + Trace-surface r12, ปอนด์):** **AU1** rewrite = audit ครอบ **"ทุกกิจกรรมที่ไม่ใช่การอ่าน" ในทุก module + login/logout/first-login-change** (การอ่าน/ดู/ค้น = ไม่ audit) + column action(event); viewer = Settings US-SET-05 (Admin only) ค้น user id/username + ช่วงวัน + filter module · **AU2** retention 1 ปี ครอบ login/logout · **AU4** Lot↔Return · **AU6 ใหม่** = trace surface (entity/topic selector + id/key + date-type + sample per object) · **A1/A6** login + password event = audit · **D-F4** trace/audit search · **§6/§7** login/logout = event ไม่ยิง noti · **§12** login-failure = audit (PO decision). ยึด `settings.md` §4d/US-SET-05 · `traceability.md` §3/§3.1/§4/§5b/§9.
- **★★ เพิ่ม (2026-07-31 — reconciliation M1+m4, ปอนด์):** **D-F2/D-F5** เพิ่มเลขใบคืน **`RET-{YYYYMM}-{NNNNNN}`** (gapless ต่อปี/เดือน, void-only; คนละ prefix กับ Route `RT-…` — แก้การชนของ token "RT") · **AU1/AU3/AU4/§10/§13** ledger source ของ `return (−)` = **RET** (แทน RT) + Lot↔Return `RET-…` + Return void-only + cross-link `return.md`/`numbering-on-save.md` · **§15 ใหม่ = Deferred-controls register** (invoice Confirmed-gate). ยึด `numbering-on-save.md` §4/§5 · `return.md` · `stock.md` §6 · `traceability.md`.
- **ไม่มีตัวเลขประดิษฐ์.**
...
|---|---|---|---|---|
| **DEF-1** | **Invoice Confirmed-gate** — สร้างใบแจ้งหนี้ได้กับ PO/SO **ทุกสถานะ** (แม้ยังไม่ Confirmed) | **ผ่อน:** ไม่ล็อกสถานะตอนสร้างใบ (create-no-status-lock) | ภายหลังจะกลับมาบังคับให้สร้าง Invoice ได้เฉพาะเมื่อ PO/SO ถึงสถานะที่กำหนด (เช่น Confirmed / ส่งสำเร็จ) | `invoice.md` §7/§10 · README §2 · permission-matrix §3.1 (r13) |
> วิธีดูแล: ทุกครั้งที่มี "การควบคุมที่จงใจผ่อน" ใหม่ ให้เพิ่มแถวที่นี่ + ชี้ไป module ต้นทาง. เมื่อ re-tighten แล้วให้ mark เป็น "CLOSED (วันที่)" ไม่ลบแถว (เก็บประวัติการตัดสินใจ).
