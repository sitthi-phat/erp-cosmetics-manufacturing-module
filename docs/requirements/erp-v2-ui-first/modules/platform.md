...
slug: `erp-v2-ui-first` · per-module canonical · PO · 2026-07-29 (**+ login lands on Dashboard / Home removed r14 2026-07-30 · ★★★★★★★ + Notification 4-type/dismiss≠read/snackbar + Forget-reset password + Account self-disable r17 2026-07-31**) · **AUTHORITATIVE SPEC** (absorbs functional-spec `platform.html` US-PLT-01..05 · **★ Settings review 2026-07-29: login basic-vs-Google choice + first-login password change**)
...
| shell ทุกหน้า | header: identity + global search + bell (panel/badge/deep link/**★ dismiss + "ดูทั้งหมด"**) · **เมนูซ้ายไม่มี "หน้าหลัก (Home)"** · **★ user menu มีปุ่ม "ปิดบัญชีของฉัน" (self-disable)** |
| **★★★★★★★ "ดูทั้งหมด" (read-all notification history)** | รายการแจ้งเตือนทั้งหมดของ user (20/หน้า, G1) — ประวัติ ไม่ใช่ active bell list |
...
| **notification** | outbox {event, **type (1–4)**, module ปลายทาง, deep link, payload, created_at} + **dismiss-bit ราย user** | computed | ผู้รับ = Read ของ module ปลายทาง · **★★★★★★★ dismiss-bit (ไม่ใช่ read/ack-bit)** · payload เช่น FG→Low แนบ Suggested |
| **badge** | number cap "9+" | computed | ราย user = จำนวน **ยังไม่ dismiss** |
...
- **★★★★★★★ Notification (bell-only, 4-type, ราย user):** event ใหม่ → เขียน outbox (J5) → fan-out ไปทุก user ที่มี **Read** ของ module ปลายทาง → ผู้รับเห็น **snackbar/toast ชั่วคราว + bell สั่น (shake)** + badge +1. **dismiss ≠ read:** คลิกปิด (dismiss) รายการ = เอาออกจาก **active bell list ของ user คนนั้นเท่านั้น** (ไม่ใช่ ack/อ่านเชิง audit; A dismiss ไม่กระทบ B) · badge = จำนวนยังไม่ dismiss · **"dismiss ทั้งหมด" = bulk-dismiss** (badge→0 ของ user นั้น) · หน้า **"ดูทั้งหมด"** = ประวัติทั้งหมด (20/หน้า) ไม่ถูกลบด้วยการ dismiss.
- **★★★★★★★ Notification 4 ประเภท (taxonomy — authoritative `non-functional.md` §7):**
  1. **งานส่งต่อข้ามแผนก (Workflow hand-off):** trigger = PO Confirmed→Production · QC ตรวจรับ RM ผ่าน/ไม่ผ่าน (→Stock) · QC Batch ผ่าน/ไม่ผ่าน · DN ส่งสำเร็จ/ลูกค้ายกเลิก/เลื่อน · PR auto-created. **ผู้รับ** = Read ของ module ปลายทาง (Production/Stock/QC/Finance/Sale/Shipping/Procurement ตาม event). **deep-link** = เอกสารปลายทางใน 그 module (เช่น PO-181 → หน้า production). **dismiss** = per-user, ตามกฎ dismiss≠read.
  2. **ความเสี่ยงสต็อก/การผลิต (Stock & Production risk):** trigger = **FG→Low** (real-time transition non-Low→Low + **J8 digest ~06:00** แนบ Suggested) · potential-delay (J4 PRD ใกล้ไม่ทันส่ง). **ผู้รับ** = Read Supply Planning (FG→Low) / Read Production+Sale+Stock (delay). **deep-link** = supply-planning / SO produce-to-stock / PRD. **dismiss** = per-user.
  3. **การเงิน/เครดิต (Finance):** trigger = Invoice **Overdue** (J3 — ส่งสำเร็จ + เลยเครดิต + ยังไม่จ่าย). **ผู้รับ** = Read Invoice (Finance+Sale). **deep-link** = invoice detail. **dismiss** = per-user.
  4. **วงจรลูกค้า (Customer lifecycle):** trigger = Customer **Inactivity** (J2 Active→Inactive). **ผู้รับ** = Read Customer (Sale ที่ดูแล). **deep-link** = customer detail. **dismiss** = per-user.
...
- **US-PLT-02 (Must) — Notification routing (4-type) ตาม Read-bit + deep link + ★★★★★★★ dismiss (≠read) ราย user + snackbar/shake:** event ใหม่ → ผู้มี Read module ปลายทางได้รับ → **snackbar โผล่ชั่วคราว + bell สั่น** → กด bell → กด "PO-181 เข้าคิวผลิต" (noti type 1) → deep link production → **กด dismiss → เอาออกจาก bell ของ user นี้ → badge ลด 1**. **Edge:** เหตุการณ์เข้า Production → เฉพาะผู้มี Read Production ได้รับ; **FG→Low (type 2) → เฉพาะ Read Supply Planning** (deep-link supply-planning + Suggested); **Overdue (type 3) → Read Invoice**; **Inactivity (type 4) → Read Customer**. **Error:** A dismiss → B เปิด bell ยังเห็น (dismiss ราย user) · **★★★★★★★ deep-link หลัง Read ถูกถอน → 403 (M-6)**.
- **US-PLT-03 (Must) — Notification panel: grouping / ★★★★★★★ dismiss + dismiss-all / "ดูทั้งหมด" / empty / badge cap:** หลาย แจ้งเตือน → panel เรียงใหม่→เก่า, กลุ่ม "ยังไม่ dismiss" ก่อน, แสดงสูงสุด N + **"ดูทั้งหมด" (read-all history list + pagination 20/หน้า)**. **Edge:** unread 12 (badge "9+") → **"ปิดทั้งหมด (dismiss all)"** → ทุกรายการ active ของ user นี้ถูก dismiss, badge→0 (ไม่กระทบผู้อื่น; **ประวัติใน "ดูทั้งหมด" ยังอยู่**); >9 แสดง "9+". **Empty:** ไม่มีแจ้งเตือน active → empty state "ไม่มีการแจ้งเตือน". **★★★★★★★ ย้ำ: dismiss = ปิดจาก bell เท่านั้น ไม่ใช่ "อ่านแล้ว/acknowledge" — ไม่มี ack=read semantics.**
...
| **★★★★★★★ dismiss / dismiss all** | เจ้าของ noti (ราย user) — **ไม่ใช่ ack/read** |
...
- badge > 9 แสดง "9+"; **★★★★★★★ dismiss เป็นราย user (ไม่ใช่ read/ack).**
...
## 9. Formulas / NFR
- notification fan-out = ผู้ใช้ทุกคนที่มี Read ของ module ปลายทาง (per-user **dismiss-bit**) — **ไม่ hardcode role**.
- **★★★★★★★ notification = bell-only, event-driven, 4-type taxonomy + dismiss≠read + snackbar/shake** — authoritative `non-functional.md` §7. **ไม่มี email notification** (email = reset password เท่านั้น, A12).
