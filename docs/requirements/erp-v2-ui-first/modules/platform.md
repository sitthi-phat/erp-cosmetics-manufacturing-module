...
slug: `erp-v2-ui-first` · per-module canonical · PO · 2026-07-29 (**+ login lands on Dashboard / Home removed r14 2026-07-30**) · **AUTHORITATIVE SPEC** (absorbs functional-spec `platform.html` US-PLT-01..05 · **★ Settings review 2026-07-29: login basic-vs-Google choice + first-login password change**)
Mockups: `mockups/login.html` · `mockups/responsive.html` (+ shell ทุกหน้า) · **landing หลัง login = `mockups/dashboard.html`** *(mockups/home.html ถูกตัดทิ้ง — Home removed 2026-07-30)*
กฎอ้างอิง: RUCDAA Read scope (noti fan-out + search + guard) · Notification matrix (continuity) · **`dashboard.md` (landing หลัง login · source เดียวของ badge/นับ)** · `settings.md` (auth/session/Google-link/password-mode provisioning) · `supply-planning.md` §5.1 + `non-functional.md` §6/§7 (FG→Low event) · Glossary · README §3

## สรุปภาษาไทย
ชั้น platform ร่วมทุกหน้า: **login — ผู้ใช้เลือก basic auth (username + password) หรือ Google login** (ปุ่ม Google ใช้ได้เฉพาะ user ที่ผูก Google account ไว้แล้ว; **★ first-login: ถ้า user โหมด "ต้องเปลี่ยนเมื่อเข้าครั้งแรก" → บังคับตั้งรหัสใหม่ก่อนใช้งาน**) · **★ login สำเร็จ → เข้าหน้า Dashboard เป็นหน้าแรก (landing)** — โมดูล Home ถูกตัดทิ้ง (2026-07-30) · identity "ESSENCE Hub System" + logo/icon + browser title ทุกหน้า · shell/นำทาง (**เมนูซ้ายไม่มี "หน้าหลัก (Home)" อีกต่อไป — Dashboard เป็น landing หลัก · งาน UX/UI sweep**) · **global header search** (ค้น PO/QT/SO/ลูกค้า/วัตถุดิบ-Lot/PR-GR/DN-Invoice ตามสิทธิ์ Read, ≥2 ตัวอักษร, ผลคลิก=deep link) · **Notification bell** (panel + badge cap "9+" + deep link + acknowledge ราย user; ผู้รับ = ผู้มีสิทธิ์ Read ของ module ปลายทาง) · **session 24 ชม. + reset 06:00 ทุกวัน (เตือนล่วงหน้าก่อนตัด)** · **responsive ทุกหน้า** + RBAC guard (เมนู/ปุ่ม/URL). notification เป็น **outbox + read-bit ราย user** (mark all read, ดูทั้งหมด, empty state) — **รวม event เชิงรุก FG→Low (Supply Planning, real-time + J8 digest, แนบ Suggested)**.
...
| หน้าจอ | บทบาท |
|---|---|
| `login.html` | เข้าสู่ระบบ — **★ ผู้ใช้เลือก basic auth (username + password) หรือ Google login** (ปุ่ม "เข้าสู่ระบบด้วย Google" ใช้ได้เฉพาะ user ที่ Admin ผูก Google account ไว้แล้ว — provisioning ที่ `settings.md`) · **★ first-login password change** (ถ้า user โหมด "ต้องเปลี่ยนเมื่อเข้าครั้งแรก") · **★ login สำเร็จ → redirect ไป Dashboard (landing)** |
| shell ทุกหน้า | header: identity + global search + bell (panel/badge/deep link/ack) · **เมนูซ้ายไม่มี "หน้าหลัก (Home)"** |
...
- **★ First-login password change:** ถ้า user มี password mode = **must-change-on-first-login** → หลัง login (basic) ครั้งแรก ระบบพาไปหน้า **ตั้งรหัสใหม่ (2 ครั้ง + show/hide)** ก่อนเข้าใช้งาน; ตั้งเสร็จ flag เคลียร์ → เข้า Dashboard. mode = **permanent** → เข้า Dashboard ได้เลย.
...
## 10. Cross-links
- ทุกการส่งงานข้าม module → noti (continuity Noti matrix) → โผล่ใน `dashboard.md` (นับ tile) + notification badge (**source เดียว · ไม่มี Home task inbox แล้ว**). **FG→Low → `supply-planning.md` §5.1 + `non-functional.md` §6 (J8)/§7.** global search ↔ ทุก module detail. **★ landing หลัง login = `dashboard.md`.** **★ auth/session/Google-link/password-mode provisioning → `settings.md` §5 US-SET-02/§4b.** Glossary (สถานะไทย).
...
- **★★ เพิ่ม (2026-07-30 — Home removed → Dashboard landing, ปอนด์ r14):** **login สำเร็จ → เข้าหน้า Dashboard เป็น landing** (แทน Home ที่ถูกตัดทิ้ง); **เมนูซ้ายตัด "หน้าหลัก (Home)"** (งาน UX/UI sweep ทุก mockup); ตัด mockup `home.html` + cross-link ไป `home.md` task inbox (ตัวเลข = source เดียวกับ notification + `dashboard.md`). §2/§3/§4/§5 US-PLT-01/US-PLT-05/§6/§7/§9/§10, ref `dashboard.md`.
