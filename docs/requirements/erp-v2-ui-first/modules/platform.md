...
กฎอ้างอิง: RUCDAA Read scope (noti fan-out + search + guard) · Notification matrix (continuity) · `home.md`/`dashboard.md` (source เดียว) · `settings.md` (auth/session) · Glossary · README §3

## สรุปภาษาไทย
ชั้น platform ร่วมทุกหน้า: **login (local + Google)** · identity "ESSENCE Hub System" + logo/icon + browser title ทุกหน้า · shell/นำทาง · **global header search** (ค้น PO/ลูกค้า/วัตถุดิบ-Lot/PR-GR/DN-Invoice ตามสิทธิ์ Read, ≥2 ตัวอักษร, ผลคลิก=deep link) · **Notification bell** (panel + badge cap "9+" + deep link + acknowledge ราย user; ผู้รับ = ผู้มีสิทธิ์ Read ของ module ปลายทาง) · **session 24 ชม. + reset 06:00 ทุกวัน (เตือนล่วงหน้าก่อนตัด)** · **responsive ทุกหน้า** + RBAC guard (เมนู/ปุ่ม/URL). notification เป็น **outbox + read-bit ราย user** (mark all read, ดูทั้งหมด, empty state).
...
| notification | outbox {event, module ปลายทาง, deep link, created_at} + read-bit ราย user | computed | ผู้รับ = Read ของ module ปลายทาง |
...
- **US-PLT-02 (Must) — Notification routing ตาม Read-bit + deep link + ack ราย user:** 5 แจ้งเตือน → กด bell → กด "PO-181 เข้าคิวผลิต" → deep link production + ack → badge เหลือ 4 (ราย user). **Edge:** เหตุการณ์เข้า Production → เฉพาะผู้มี Read Production ได้รับ. **Error:** A ack → B เปิด bell ยังเห็น (ack ราย user).
...
- **US-PLT-04 (Should) — Global header search:** พิมพ์ "181"/"กลอรี่" → ผลจัดกลุ่มตามชนิด (PO/ลูกค้า/วัตถุดิบ-Lot/PR-GR/DN-Invoice); คลิก = deep link. **Edge:** ผลนอกสิทธิ์ Read ไม่ปรากฏ; ไม่พบ → "ไม่พบผลการค้นหา". **Error:** <2 ตัวอักษร → ไม่ยิงค้น + "พิมพ์อย่างน้อย 2 ตัวอักษร".
...
| รับ notification | **Read (R)** ของ module ปลายทาง (fan-out ตาม Read-bit) |
...
## 9. Formulas / NFR
- notification fan-out = ผู้ใช้ทุกคนที่มี Read ของ module ปลายทาง (per-user read-bit).
- session: 24 ชม. + daily reset 06:00 + pre-expiry warning.
- NFR: responsive ทุกหน้า (Must) · local + Google · 50 concurrent · < 2s (hard 3s).

## 10. Cross-links
- ทุกการส่งงานข้าม module → noti (continuity Noti matrix) → โผล่ใน `home.md` task inbox + `dashboard.md` badge (source เดียว). global search ↔ ทุก module detail. auth/session config → `settings.md`. Glossary (สถานะไทย).

## 11. Module changelog
- **Absorbed:** functional-spec `platform.html` US-PLT-01..05 (15 AC) verbatim ในความหมาย.
- **คงเดิม:** identity ทุกหน้า · noti outbox + read-bit ราย user (routing ตาม Read, deep link, ack, mark all read, badge "9+", empty) · global search ≥2 ตัวอักษร ตามสิทธิ์ · session 24h/06:00 + warning · responsive + RBAC guard.
